...
public class SysCityService {
     ...
     * @return
     */
    public PageInfo<SysCity> list(Tablepar tablepar, String searchTxt, String searchText) {
        SysCityExample testExample = new SysCityExample();
        testExample.setOrderByClause("id ASC");
        if (searchTxt != null && !"".equals(searchTxt)) {
            testExample.createCriteria().andCityNameLike("%" + searchTxt + "%");
        }
        if (searchText != null && !"".equals(searchText)) {
            testExample.createCriteria().andCityNameLike("%" + searchText + "%");
        }
