...
public class TSysEmailService implements BaseService<TSysEmail, TSysEmailExample> {
     ...
     * @return
     */
    public PageInfo<TSysEmail> list(Tablepar tablepar, String searchTxt, String searchText) {
        TSysEmailExample testExample = new TSysEmailExample();
        testExample.setOrderByClause("id desc");
        if (searchTxt != null && !"".equals(searchTxt)) {
            testExample.createCriteria().andContentLike("%" + searchTxt + "%");
        }
        if (searchText != null && !"".equals(searchText)) {
            testExample.createCriteria().andContentLike("%" + searchText + "%");
        }
